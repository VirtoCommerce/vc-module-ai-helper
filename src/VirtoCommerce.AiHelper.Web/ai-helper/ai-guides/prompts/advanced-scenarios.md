...
const grandTotal = computed(() => subtotal.value + tax.value + (order.value.shipping || 0));
...
const canSave = computed(() => !isValidating.value && Object.keys(errors.value).length === 0);
```
...
const performTransition = async (targetState: DocumentState, comment?: string) => {
  ...
  const transition = transitions.find((t) => t.from === document.value.state && t.to === targetState);
