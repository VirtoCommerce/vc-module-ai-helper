...
```vue
<VcInput v-model="name" label="Name" placeholder="Enter name" :required="true" />
```
...
```vue
<VcInputDropdown v-model="search" :options="options" placeholder="Search..." />
```
...
```vue
<VcPagination :pages="totalPages" :current-page="currentPage" @item-click="handlePageChange" />
```
...
```vue
<VcRadioButton v-model="selected" value="option1" label="Option 1" />
```
...
```vue
<VcSelect v-model="category" :options="categories" label="Category" searchable />
```
...
```vue
<VcTextarea v-model="description" label="Description" :rows="5" />
```
...
```vue
<VcBlade :expanded="true" :closable="true" :param="id" :options="undefined" title="Details">
  Content
</VcBlade>
```
...
```vue
<VcDynamicProperty :properties="properties" v-model="values" />
```
...
```vue
<VcTable :items="items" :columns="columns" :loading="loading" @item-click="handleItemClick" />
```
